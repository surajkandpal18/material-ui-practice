...
export default function Header(props){
  ...
  {name:'Custom Software Development',link:'/customsoftwares',activeIndex:1,selectedIndex:1},
  ...
  const tabs=(
  	<React.Fragment>
		...
		<Button className={classes.button} component={Link} to='/estimate' onClick={()=>{props.setValue(5)}} variant='contained' color='secondary'>
					Free Estimate
		</Button>
				<Menu id="simple-menu"
					  anchorEl={anchorEl}
  ...
  const drawer=(
  	<React.Fragment>
  	<SwipeableDrawer classes={{paper:classes.drawer}} disableBackdropTransition={!iOS} disableDiscovery={iOS} open={openDrawer} onClose={()=>setOpenDrawer(false)} 
  	...
  	{routes.map((route,index)=>(
  		<ListItem key={`${route}${route.activeIndex}`} 
  			...
  			<ListItemText
  						 ...
  						 disableTypograph>{route.name}</ListItemText>
  		</ListItem>
  	))}
	...
	return(
		...
		<AppBar position="fixed" color='primary' className={classes.appBar}>
			<Toolbar disableGutters>
				...
				<img src={logo} alt='compantlogo' className={classes.logo}/>
				</Button>
				{matches?drawer:tabs}
